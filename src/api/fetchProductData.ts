import type Product from '../types/model.type'
...
export default async function fetchProductData(
  ...
  try {
    ...
    if (params.minPrice !== undefined) queryParams.append('price_gte', params.minPrice.toString())

    if (params.maxPrice !== undefined) queryParams.append('price_lte', params.maxPrice.toString())
    ...
    if (params.brandSelected && params.brandSelected.length) {
      params.brandSelected.forEach((brand) => queryParams.append('brand', brand))
    }
